...
export const newsApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getBysources: builder.query<Article[], { sources: string; count: number }>({
      query: ({ sources, count }) => `/news/sources?sources=${sources}&count=${count}`,
    }),
  }),
});

export const { useGetheadlinesQuery, useGetByCategoryQuery, useGetBysourcesQuery, useGetTodayNewsQuery } = newsApiSlice;

export const weatherApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getWeather: builder.query<RootWeather, { longitude: number | null; latitude: number | null }>({
